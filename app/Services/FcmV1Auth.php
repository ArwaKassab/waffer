...
class FcmV1Auth
{
    public function getAccessToken(): string
    {
        return Cache::remember('fcm_v1_access_token', now()->addMinutes(50), function () {
            ...
            $json = (string)config('services.fcm_v1.service_account_json');
            ...
            if ($json) {
                ...
                if ($file !== '' && !$this->isAbsolutePath($file)) {
                    // لو مسار نسبي: خليه مطلق بناءً على مجلد المشروع
                    $file = base_path($file);
                }

                if ($file === '' || !is_file($file) || !is_readable($file)) {
                    throw new \RuntimeException(
                        'FCM: لم يتم ضبط بيانات الخدمة (لا JSON ولا FILE). file=' . ($file ?: 'NULL')
                    );
                }
